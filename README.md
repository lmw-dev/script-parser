...
**症状**：报错 `URL解析失败` 或 `从 HTML 中解析视频信息失败`

**原因**：抖音视频 URL 解析需要访问海外 CDN 服务，在某些网络环境下可能被干扰或限速。

**解决方案**：
- ✅ **推荐**：使用腾讯云 VPS 等公有云服务器部署（网络环境更优）
- ⚠️ **Mac mini 开发环境**：可能需要配置代理或使用腾讯云服务来测试
...
| 环境 | 抖音解析 | 说明 |
|------|----------|------|
| **腾讯云 VPS** | ✅ 正常 | 公有云网络环境优，推荐生产环境 |
| **Mac mini（家庭网络）** | ⚠️ 可能失败 | 适合开发测试，不建议作为生产环境 |
| **本地 MacBook** | ⚠️ 需要代理 | 开发环境，可配置 VPN/代理 |
