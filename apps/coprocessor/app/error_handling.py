...
class ErrorHandler:
    ...
    ERROR_MAPPINGS: Dict[type, tuple[int, int, str]] = {
        URLParserError: (400, ErrorMapping.URL_PARSER_ERROR, "Failed to parse video URL"),
        ASRError: (503, ErrorMapping.ASR_ERROR, "ASR service is temporarily unavailable"),
        LLMError: (502, ErrorMapping.LLM_ERROR, "LLM service error occurred"),
        FileHandlerError: (500, ErrorMapping.FILE_HANDLER_ERROR, "File processing error"),
        OSSUploaderError: (503, ErrorMapping.OSS_UPLOADER_ERROR, "OSS service is temporarily unavailable"),
    }
    ...
    def create_error_response(
        ...
        if exception_type in cls.ERROR_MAPPINGS:
            http_status, business_code, user_message = cls.ERROR_MAPPINGS[exception_type]
        else:
...
def create_json_decode_error(start_time: Optional[float] = None) -> HTTPException:
    """
    ...
    """
    return ErrorHandler.create_validation_error("Invalid JSON format", start_time)


def create_missing_input_error(start_time: Optional[float] = None) -> HTTPException:
    """
...
def create_form_url_error(start_time: Optional[float] = None) -> HTTPException:
    """
